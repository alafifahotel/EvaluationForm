...
class GitHubService {
  ...
  async loadProcedures() {
    try {
      ...
      console.error('Error loading procedures from GitHub:', error)
      throw error
    }
  }

  async saveProcedures(data, sha = null) {
    try {
      // Ensure the config directory exists
      await this.ensureDirectoryExists('config')
      ...
      const params = {
        ...
        repo: this.repo,
        path: 'config/procedures.json',
        message: `Update procedures manual - ${format(new Date(), 'dd/MM/yyyy HH:mm')}`,
        content: content,
        branch: this.branch
      }
      ...
      console.error('Error saving procedures to GitHub:', error)
      throw error
    }
  }
}
